...
class locusEmptyClass:
    def __init__(self):
        self.featureDict = {"type": "Feature", "properties": {}}
...
def json_dump(request):
    ...
    return HttpResponse(
        simplejson.dumps(dumpJson.featureDict, sort_keys=True, indent=4 * " "),
...
def single_json_dump(request, id):
    ...
    return HttpResponse(
        simplejson.dumps(f, sort_keys=True, indent=4 * " "),
        content_type="application/json",
    )
...
def create_geojson_locus(locus):
    ...
    for n in locus.variants.all():
        jsonObj["properties"]["name_variants"].append(
            {"variant": n.name, "attestation": n.attestation, "language": n.language}
        )
